...
#         kmeans = faiss.Kmeans(tf.shape[1], len(tf)//factor, niter=300, nredo=5, gpu=True, seed=args.seed)
    ...
    if args.mode == '3shot':
        ...
        b.train()
        c.train()
        class_soft_labels = np.load(f'data/labels/custom_soft_labels/s{args.source}_t{args.target}_3.npy')
        ...
        output_path = Path(f'./data/{args.dataset["name"]}/3shot/res34/s{args.source}_t{args.target}_{args.seed}/custom_label_smoothing_3.npz')
