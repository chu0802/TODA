...
#         kmeans = faiss.Kmeans(tf.shape[1], len(tf)//factor, niter=300, nredo=5, gpu=True, seed=args.seed)
    ...
    if args.mode == '3shot':
        ...
        dim = 100
        label_correction_soft_labels = np.load(f'data/labels/label_correction_soft_labels/s{args.source}_t{args.target}_{dim}.npy')
        ...
        save(f'{args.dataset["name"]}/3shot/res34/s{args.source}_t{args.target}_{args.seed}/label_correction_soft_labels_{args.alpha}_{args.num_iters}_{dim}.pt', f=f, b=b, c=c)
